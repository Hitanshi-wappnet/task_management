...
class TaskView(APIView):
    ...
    def delete(self, request):
        ...
        if user.groups.filter(name='Manager').exists():
            ...
            # retrieve the title entered by user
            title = request.data.get("title")

            # If the title is None then returns an response
            if title is None:
                response = {
                    "status": False,
                    "message": "Provide title of task which you want to delete",
                    "data": None
                }
                return Response(data=response,
                                status=status.HTTP_400_BAD_REQUEST)
            ...
            # returns error if Task with given title does not exist
                ...
                response = {
                    "status": False,
                    "message": "Task with given title does not exist",
                    "data": None
                }
                return Response(data=response, status=status.HTTP_200_OK)
